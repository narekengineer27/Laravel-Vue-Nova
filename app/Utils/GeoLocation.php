...
class GeoLocation {
    ...
    public static function getElasticSearchBounding($lat, $lon, $radius)
    {
        $coordinates = self::fromDegrees($lat, $lon)->boundingCoordinates($radius, "km");
        $bottomLeft = $coordinates[0];
        $topRight = $coordinates[1];
        return [
            'bottom_left' => [
                "lat" => $bottomLeft->degLat, "lon" => $bottomLeft->degLon
            ],
            'top_right' => [
                "lat" =>  $topRight->degLat, "lon" => $topRight->degLon
            ]
        ];
    }
